...
int GetDeviceInformation(int device_id, DeviceInformation *info) {
  ...
  // Next, jump through a bunch of hoops to read the current GPU clock on the
  // GPU.
  if (!CheckHIPError(hipMalloc(&device_clock, sizeof(uint64_t)))) return 0;
  hipLaunchKernelGGL(GetCurrentClock, 1, 1, 0, 0, device_clock);
  if (!CheckHIPError(hipDeviceSynchronize())) {
    hipFree(device_clock);
    return 0;
  }
  if (!CheckHIPError(hipMemcpy(&(info->starting_clock), device_clock,
