...
def get_total_kernel_times(plugin):
    ...
    for t in plugin["times"]:
        ...
        #    continue
        #first_block_start = t["block_times"][0]
        #last_block_end = t["block_times"][-1]
        #current_total += last_block_end - first_block_start
...
def plot_scenario(plugins, name, times_key):
    ...
    labels = []
    c = 0
    plugins = [plugins[2], plugins[3], plugins[0], plugins[1]] ###################################################################################### PAPER
    for b in plugins:
        c += 1
